...
**Deterministic logic > AI generation.** Because EnvForage generates scripts that run on user systems, we take security exceptionally seriously.

## Supported Versions

EnvForage provides security updates for the following versions:
...
3. The affected versions or components (e.g., `TemplateRenderer`, `envforage`).
4. Any potential mitigations you suggest.

## Unsafe Command Policy

EnvForage explicitly forbids the generation of dangerous shell commands. Every generated script passes through a strict `SafetyFilter`. We consider any bypass of this filter a critical security vulnerability.
...
EnvForage is designed to *provision* and *repair*, not to blindly destroy.
...
Every script generated by EnvForage is validated by the `SafetyFilter`
