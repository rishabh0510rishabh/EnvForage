...
MOCK_LOCATIONS = [
    Location(
        id="1", name="Central Park Foraging Spot", latitude=40.7812, longitude=-73.9665
    ),
...
async def get_nearby_locations(
    latitude: float = Query(
        ..., ge=-90, le=90, description="Latitude of the center point"
    ),
    longitude: float = Query(
        ..., ge=-180, le=180, description="Longitude of the center point"
    ),
    radius_km: float = Query(
        5.0,
        gt=0,
        le=MAX_SEARCH_RADIUS_KM,
        description=f"Search radius in km. Max {MAX_SEARCH_RADIUS_KM}km to prevent DB locks.",
    ),
):
    """
    ...
    for loc in bbox_filtered:
        ...
        if distance <= radius_km:
            results.append({"location": loc, "distance_km": round(distance, 2)})
