...
class PaginationEngine:
    ...
    @staticmethod
    def apply_cursor(
        stmt: Select, sort_column, cursor_value: str | None, sort_desc: bool = True
    ):
    ...
    @staticmethod
    def build_response(
        items: list[Any], limit: int, cursor_attr: str  # noqa: F821
    ) -> CursorPagination:
        has_more = len(items) > limit
        if has_more:
            items = items[:limit]
            last_item = items[-1]
            next_cursor = PaginationEngine.encode_cursor(
                str(getattr(last_item, cursor_attr))
            )
        else:
            next_cursor = None
