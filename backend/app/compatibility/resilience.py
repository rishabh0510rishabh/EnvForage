...
class CircuitBreaker:
    ...
    def record_failure(self):
        ...
        if self.state == CircuitState.HALF_OPEN:
            ...
            logger.error(
                f"Circuit failure threshold reached ({self.failure_count}). Tripping OPEN."
            )
            self.state = CircuitState.OPEN
        ...
        **kwargs,
        ...
        while True:
            try:
                ...
                logger.warning(
                    f"Execution failed: {e}. Retrying in {delay}s ({retries}/{max_retries})..."
                )
        ...
        async def wrapper(*args, **kwargs):
            if not matrix_circuit_breaker.can_execute():
                raise CircuitBreakerError(
                    "Circuit is OPEN. Fast-failing request to protect downstream."
                )

            try:
                result = await ExponentialBackoff.execute(
