...
def _restricted_execution_worker(
    plugin_code: str, context: dict[str, Any], result_queue: multiprocessing.Queue
):
    """
    ...
    try:
        ...
        if (
            "import os" in plugin_code
            or "import sys" in plugin_code
            or "__import__" in plugin_code
        ):
    ...
    def execute(self, plugin_code: str, context: dict[str, Any] = None) -> Any:
        ...
        try:
            ...
            if response["status"] == "success":
                logger.debug(
                    f"Sandbox execution successful in {time.time() - start_time:.3f}s"
                )
                return response["result"]
            else:
                raise SandboxExecutionError(
                    f"{response['type']}: {response['message']}"
                )
            ...
            raise SandboxTimeoutError(
                f"Plugin execution exceeded maximum allowed time of {self.timeout_seconds}s"
            )
