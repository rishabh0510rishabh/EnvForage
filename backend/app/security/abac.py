...
class ABACEvaluator:
    ...
    def _evaluate_condition(
        self, condition: PolicyCondition, subject: Subject, resource: Resource
    ) -> bool:
    ...
    def is_allowed(self, subject: Subject, action: str, resource: Resource) -> bool:
        ...
        for policy in self.policies:
            # Check subject match
            subj_match = "*" in policy.subjects or any(
                r in policy.subjects for r in subject.roles
            )
            if not subj_match:
                continue
            ...
            # Check conditions
            cond_match = all(
                self._evaluate_condition(c, subject, resource)
            ...
            if policy.effect == "deny":
                logger.warning(
                    f"ABAC Deny: {subject.id} attempted {action} on {resource.type}/{resource.id} (Policy {policy.id})"
                )
        ...
        if allowed:
            logger.debug(
                f"ABAC Allow: {subject.id} -> {action} -> {resource.type}/{resource.id}"
            )
        else:
            logger.warning(
                f"ABAC Default Deny: {subject.id} -> {action} -> {resource.type}/{resource.id}"
            )
...
GLOBAL_POLICIES = [
    ...
    ABACPolicy(
        ...
        conditions=[
            PolicyCondition(
                attribute="resource.owner_id", operator="eq", value="subject.id"
            )
        ],
    ),
]
