...
class CacheService:
    ...
    async def get(self, key: str) -> Any | None:
        if self.redis:
            try:
                ...
                logger.warning(
                    f"Redis get failed for {key}: {e}. Falling back to memory."
                )
    ...
    async def set(self, key: str, value: Any, ttl_seconds: int = 3600):
        ...
        if self.redis:
            try:
                ...
                logger.warning(
                    f"Redis set failed for {key}: {e}. Falling back to memory."
                )
    ...
    async def get_or_set(
        self, key: str, fetch_func: Callable, ttl_seconds: int = 3600
    ) -> Any:
