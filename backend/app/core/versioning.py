        ...
        def decorator(func: Callable) -> Callable:
            ...
            async def wrapped_endpoint(request: Request, *fn_args, **fn_kwargs):
                client_version = request.headers.get(
                    "Accept-Version", self.current_version
                )

                if (
                    client_version
                    not in [self.current_version] + self.deprecated_versions
                ):
                    raise HTTPException(
                        status_code=400,
                        detail=f"Unsupported API version: {client_version}",
                    )
                ...
                if hasattr(response, "headers"):
                    if client_version in self.deprecated_versions:
                        response.headers["Warning"] = (
                            f'299 - "API version {client_version} is deprecated"'
                        )
